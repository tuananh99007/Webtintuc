        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <div class="table-responsive">
                    <table class="table table-striped table-bordered table-hover">
                        ...
                        @foreach ($productsList as $index => $productList)
                            <tr id="product-{{ $productList->id }}">
                                ...
                                <td><img src="{{ asset($productList->image) }}" alt="" style="height: 60px "></td>
                                <td>{{ $productList->category_name }}</td>
                                <td>{{ $productList->user_id }}</td>
                                <td>{{ $productList->post_time }}</td>
                    ...
                    <a class="btn btn-danger" href="{{ route('admin.product.detailBin') }}">Thùng rác</a>
                </div>
            </div>
        </div>
    </div>
    <script>
        $(document).ready(function () {
            $(".event-delete").click(function () {
                swal({
                    title: "Are you sure?",
                    text: "Nếu xóa sản phẩm sẽ đi vào thùng rác",
                    icon: "warning",
                    buttons: true,
                    dangerMode: true,
                })
